...
The first step in real-world analysis is to segment the ECG signals into isolated heartbeats. This requires detecting **R-peaks** from the raw signal. Although many algorithms exist—such as Christov, Pan–Tompkins, and [NeuroKit2](https://github.com/neuropsychology/NeuroKit)—a [recent study of ours](https://github.com/your-repo-link) found NeuroKit2 to be the most accurate. Since MIT-BIH already provides annotated R-peak locations, we use these directly.
