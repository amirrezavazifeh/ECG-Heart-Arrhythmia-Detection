...
Both methods are neighbor-embedding techniques that aim to preserve the structure of high-dimensional data: points that are close in the original space remain close in the low-dimensional embedding, while distant points are placed far apart. Compared to t-SNE, UMAP is faster, preserves more global structure, and scales better to large datasets.
